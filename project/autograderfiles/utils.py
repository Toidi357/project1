...
class ProcessRunner():
    def __init__(self, cmd, data, stderr_file=None):
        ...
        f.write(data)
        os.system(f'chmod o+r /tmp/{filename}')
        f.close()
        self.f = open(f'/tmp/{filename}', 'rb')

        if stderr_file:
            self.stderr_file = open(f'/autograder/results/{stderr_file}', 'wb')
        else:
            self.stderr_file = open(f'/autograder/results/err{randomword(10)}', 'wb')
...
def proxy(client_port, server_port, loss_rate, reorder_rate, corrupt_rate):
    random.seed(4322)
    ...
    while True:
        try:
            ...
            if random.random() < corrupt_rate and count1 > 3:
                data = flip_bit(data)
        ...
        try:
            ...
            if random.random() < corrupt_rate and count2 > 3:
                data = flip_bit(data)
